...
void FlowMonitorHelper::SetMonitorAttribute(std::string n1,
		const AttributeValue &v1) {
	m_monitorFactory.Set(n1, v1);
}

Ptr<FlowMonitor> FlowMonitorHelper::GetMonitor() {
	if (!m_flowMonitor) {
...
Ptr<FlowClassifier> FlowMonitorHelper::GetClassifier() {
	if (!m_flowClassifier) {
		m_flowClassifier = Create<Ipv4FlowClassifier>();
	}
...
Ptr<FlowMonitor> FlowMonitorHelper::Install(Ptr<Node> node) {
	...
	Ptr<Ipv4FlowProbe> probe = Create<Ipv4FlowProbe>(monitor,
			DynamicCast<Ipv4FlowClassifier>(classifier), node);
	return m_flowMonitor;
}

Ptr<FlowMonitor> FlowMonitorHelper::Install(NodeContainer nodes) {
