// Modified by Chunzhi on Feb. 21, 2013
// Add various ECMP routing support.
//
...
private:
	...
	// parameters 'header' and 'iif' are added by Chunzhi
	...
	/*--------------------------------Chunzhi----------------------------------*/
	uint32_t m_ecmpStrategyId; // by Chunzhi
	std::string m_output_file; // by Chunzhi
	std::map<std::string, uint32_t> flowRouteMap; // by Chunzhi
	std::map<int, int> indexFlowsMap; // Chunzhi
	uint32_t m_roundRobin_pointer; // Chunzhi
	uint32_t m_ecmpRandom_seed; //Chunzhi
	std::vector<uint> vector_rr; //Chunzhi
	std::map<uint, std::vector<uint> > iif_vectorRR; //Chunzhi

	/**
	 * May need to scan these new APIs. (added by Chunzhi)
	 *
	 * To rebuild python API binging:
	 * (1) install "python-pygccxml" and "CMake";
	 * (2) run "./waf configure", then run "./waf --apiscan=all" to scan all bindings (takes long time), or "./waf --apiscan=modulename" for a specific modulename.
	 */
	virtual uint32_t
	GetECMPIndex(Ptr<const Packet> p, const Ipv4Header &header, uint nRoutes,
			uint32_t iif);

	/// ecmp routing strategies, by Chunzhi
	uint32_t EcmpPacketRandom(uint32_t nRoutes);
	uint32_t EcmpPacketRoundRobin(uint32_t nRoutes, uint32_t iif);
	uint32_t EcmpPacketCycle(uint32_t nRoutes, uint32_t iif);
	uint32_t EcmpFlowRandom(Ptr<const Packet> p, const Ipv4Header &header,
			uint32_t nRoutes);
	uint32_t EcmpFlowCycle(Ptr<const Packet> p, const Ipv4Header &header,
			uint32_t nRoutes);
