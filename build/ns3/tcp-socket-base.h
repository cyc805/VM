...
//	TcpRxBuffer GetRxBuffer(void); // by Chunzhi
...
protected:
	...
	virtual void DoForwardUp(Ptr<Packet> packet, Ipv4Header header,
			uint16_t port, Ptr<Ipv4Interface> incomingInterface); //Get a pkt from L3
	virtual void DoForwardUp(Ptr<Packet> packet, Ipv6Header header,
			uint16_t port); // Ipv6 version
	...
	uint32_t SendDataPacket(SequenceNumber32 seq, uint32_t maxSize,
			bool withAck); // Send a data packet
	...
	void ProcessListen(Ptr<Packet>, const TcpHeader&, const Address&,
			const Address&); // Process the newly received ACK
	void ProcessSynSent(Ptr<Packet>, const TcpHeader&); // Received a packet upon SYN_SENT
	void ProcessSynRcvd(Ptr<Packet>, const TcpHeader&, const Address&,
			const Address&); // Received a packet upon SYN_RCVD
...
protected:
	...
	/*-------------------------Chunzhi-------------------------------*/
//	std::string m_reordering_delay_file; // by Chunzhi
//	Time m_collectStartTime; // by Chunzhi
	int m_maxSeqGap; // by Chunzhi. A (maximum) threshold (in number of packets or sequence number gap) to identify congestion

	int m_seqGap; // by Chunzhi. sequence number gap for different hop count flows.
	uint32_t GetHopCount();
	uint32_t GetSeqGap();

//	Time m_suppressTime; // by Chunzhi
//	Time m_maxSuppressTime;
//	EventId m_STOEvent;
//	void SuppressTimeOut(void);

//	std::stringstream m_ackTraceStream;
//	std::string m_ackFileName;
//	void SetAckFile(std::string fileName);
//	void PrintAck(void);
//	void RecordAck(std::string ack_type, SequenceNumber32 ackNumber);

//	std::stringstream m_seqNumStream;
//	std::string m_seqNumFileName;
//	void SetSeqNumFile(std::string fileName);
//	void PrintSeqNum(void);
//	void RecordSeqNum(std::string position, SequenceNumber32 seqNum);

//	std::ofstream m_traceFileStream; //

	// ECN related attributes
	bool m_ECN;         //< Enable ECN or not
...
enum EcnState {
	NO_ECN = 0x0, ECN_CONN = 0x1,     //< ECN Connection is established
