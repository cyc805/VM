...
public:
	...
	struct FlowStats {
		...
		/**-------------------------------Chunzhi------------------------**/
		// data for calculating hop-by-hop average packet delay.
		// key: current hop count.
		/// pair_1: delay sum of this flow at current hop
		/// pair_2: total number of received packets of this flow at current hop
		/// pair_1/pair_2: average packet delay of this flow at current hop
		std::map<uint32_t, Time> hop_delaySum;
		std::map<uint32_t, uint32_t> hop_nPackets;

		// route: "1->2->3".
		// nPkts: number of packets passing through this route.
		std::map<std::string, uint32_t> route_nPkts;
	...
	void ReportFirstTx(Ptr<FlowProbe> probe, FlowId flowId,
	...
	void ReportLastRx(Ptr<FlowProbe> probe, FlowId flowId,
			FlowPacketId packetId, uint32_t packetSize);
	/// FlowProbe implementations are supposed to call this method to
	...
	std::string SerializeToXmlString(int indent, bool enableHistograms,
			bool enableProbes);
	...
	void ReportDropPkt(uint32_t nodeId, Ptr<Ipv4FlowClassifier> classifier,
			FlowId flowId, Ptr<Packet> pkt);
	void RecordFlowStats();
	void RecordFlowStat(FlowId flowId);
	void RecordPkt(uint32_t nodeId, Ptr<Ipv4FlowClassifier> classifier,
			FlowId flowId, const Ipv4Header &ipHeader, Ptr<Packet> pkt,
			std::string event);
	/*-------------------------------------------------------------------*/
...
private:
	...
	struct TrackedPacket {
		...
		// Chunzhi
		std::vector<uint32_t> path; // record the path (a list of Node Ids) that the packet pass through.
	};

public:
	// by Chunzhi: change from private to public
	// FlowId --> FlowStats
...
private:
	...
	/*--------------------------------Chunzhi------------------------------*/
	Time m_collectStartTime; // by Chunzhi
	std::stringstream m_dropPktStream; // Chunzhi
	bool m_enableMyDropTrace; // Chunzhi
	bool m_enablePathTrace; // Chunzhi
	Ptr<MyOutputStreamWrapper> m_flowStat_output_stream;
	Ptr<MyOutputStreamWrapper> m_event_output_stream;
//	Time m_endTime;
//	Ptr<Ipv4FlowClassifier> m_ipv4_fc;

public:
//	Ptr<MyFlowClassifierWrapper> m_myFCW;
	Ptr<FlowClassifier> GetFlowClassifier(void);
//	void Clear(void);
	/*---------------------------------------------------------------------*/
};
