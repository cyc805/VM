...
private:
	...
	/*----------------------by Chunzhi------------------------*/
	uint32_t TotalRunNum; // Set the total number of running BulkSendApplications
	uint32_t m_ReTxThreshold;
	Ptr<Socket> m_mySocket; // custom socket to replace m_socket

	// for testing TCP incast
	bool m_synchronize_mode;
...
private:
	...
	/*----------------------------by Chunzhi----------------------*/
public:
	static void ReStartBulkSendApps(uint nSender); // by Chunzhi
	bool GetConnectionStat(); // by Chunzhi
	void SetConnectionStat(bool status);
	/*------------------------------------------------------------*/
};
